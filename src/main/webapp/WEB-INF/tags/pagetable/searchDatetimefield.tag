...
<c:if test="${searchBar}">	
<label><b>${title}:</b> <input id="${id}" class="laydate-icon input-medium" name="SEARCH_${key}@=@DATE" type="SEARCH" class="laydate-icon input-medium" placeholder="YYYY-MM-DD hh:mm:ss" value="${value}" onclick="laydate({istime: true, festival: true, format: 'YYYY-MM-DD hh:mm:ss'})"> </label>	&nbsp;&nbsp;&nbsp;&nbsp;
</c:if>
