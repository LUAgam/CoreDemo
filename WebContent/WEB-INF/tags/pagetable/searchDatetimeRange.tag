...
<c:if test="${searchBar}">	
<label><b>${title}:</b> <input id="start_${id}" name="SEARCH_${key}@>=@DATERANGE" type="SEARCH" class="laydate-icon input-medium" placeholder="YYYY-MM-DD hh:mm:ss" value="${startDate}" onclick="laydate({istime: true, festival: true, format: 'YYYY-MM-DD hh:mm:ss'})"> <input id="END_${id}" name="SEARCH_${key}@<@DATERANGE" type="SEARCH" class="laydate-icon input-medium" placeholder="YYYY-MM-DD hh:mm:ss" value="${endDate}" onclick="laydate({istime: true, festival: true, format: 'YYYY-MM-DD hh:mm:ss'})"> </label>&nbsp;&nbsp;&nbsp;&nbsp;
</c:if>
