...
<!-- rtexprvalue = "false"时，此属性的值，只能直接指定-->
...
<c:if test="${hasDatefield eq false}">
	<c:set var="hasDatefield" value="true" scope="request" />
</c:if>
		...
		<div class="input-append">
			<c:choose>
				...
				<c:otherwise>
					<input class="laydate-icon ${size}" id="${id}" name="${id}"
						value='<fmt:formatDate value="${value}" pattern="yyyy-MM-dd"/>'
						 type="text" onclick="laydate({istime: false, festival: true, format: 'YYYY-MM-DD'})"></input>
				</c:otherwise>
			</c:choose>
		</div>
